...
private data class Age1720AssessmentChunk(val id: Int, val text: String)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun Age1720AssessmentScreen(
    isListening: Boolean,
    speak: (String) -> Unit,
    listen: ((String) -> Unit) -> Unit,
    onReviewResult: (Int, ReviewKind, Boolean) -> Unit,
    onFinish: (Age1720AssessmentSummary) -> Unit,
    onBack: () -> Unit
) {
    ...
    val summary = Age1720AssessmentSummary(
        listeningCorrect,
        speakingCorrect,
        orderCorrect,
        situationCorrect
    ...
    Scaffold(
        ...
        val question = Age1720AssessmentCatalog.questions[currentIndex]
        ...
        Column(
            ...
            LinearProgressIndicator(
                progress = (currentIndex + 1).toFloat() / Age1720AssessmentCatalog.questions.size,
                modifier = Modifier.fillMaxWidth()
            )
            Text(
                "문항 ${currentIndex + 1} / ${Age1720AssessmentCatalog.questions.size}",
            ...
            when (question.kind) {
                AssessmentKind.LISTENING -> Age1720ListeningQuestion(
                    lesson = lesson,
                    answered = answered,
                    speak = speak,
                    onAnswer = { correct ->
                ...
                AssessmentKind.SPEAKING -> Age1720SpeakingQuestion(
                    lesson = lesson,
                    answered = answered,
                    isListening = isListening,
                    listen = listen,
                    threshold = 75,
                    prompt = "한국어 의도를 보고 자연스럽게 영어로 말하세요.",
                    onAnswer = { correct ->
                        if (!answered) {
                            answered = true
                            success = correct
                ...
                AssessmentKind.ORDER -> Age1720OrderQuestion(
                    lesson = lesson,
                    answered = answered,
                    onAnswer = { correct ->
                        if (!answered) {
                ...
                AssessmentKind.SITUATION -> Age1720SituationQuestion(
                    lesson = lesson,
                    answered = answered,
                    isListening = isListening,
                    listen = listen,
            ...
            if (answered) {
                Card(
                    ...
                    Column(
                        ...
                        if (guide.rhythmEnglish.isNotBlank()) {
                            Text(guide.rhythmEnglish, fontSize = 17.sp)
                            Text(guide.rhythm, fontSize = 14.sp)
                        }
                    }
                }
                ...
                Button(
                    onClick = {
                        if (currentIndex == Age1720AssessmentCatalog.questions.lastIndex) {
                            finished = true
                        } else {
                            currentIndex += 1
                        }
...
@Composable
private fun Age1720ListeningQuestion(
    lesson: Lesson,
    answered: Boolean,
    speak: (String) -> Unit,
    ...
    val choices = remember(lesson.id) { age1720MeaningChoices(lesson) }
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            Text("핵심 내용뿐 아니라 조건, 태도, 뉘앙스까지 들어보세요.")
            Button(
                onClick = {
                    if (playCount < 2) {
                        playCount += 1
...
@Composable
private fun Age1720SpeakingQuestion(
    lesson: Lesson,
    answered: Boolean,
    isListening: Boolean,
    listen: ((String) -> Unit) -> Unit,
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            Text(prompt, fontSize = 19.sp, fontWeight = FontWeight.Bold)
            Text(lesson.meaning, fontSize = 21.sp, fontWeight = FontWeight.Bold)
            Button(
                onClick = {
                    listen { text ->
                        recognized = text
                        score = age1720SentenceMatch(text, lesson)
                        onAnswer(score >= threshold)
                    }
                },
...
@Composable
private fun Age1720OrderQuestion(
    lesson: Lesson,
    answered: Boolean,
    onAnswer: (Boolean) -> Unit
) {
    val chunks = remember(lesson.id) { age1720Chunks(lesson) }
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            Text(lesson.meaning, fontSize = 21.sp, fontWeight = FontWeight.Bold)
            Text(
                if (selected.isEmpty()) "아래 덩어리를 순서대로 선택하세요."
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                OutlinedButton(
                    onClick = { if (selectedIds.isNotEmpty()) selectedIds = selectedIds.dropLast(1) },
                ...
                Button(
                    onClick = {
                        ...
                        onAnswer(age1720Normalize(candidate) == age1720Normalize(lesson.target))
                    },
...
@Composable
private fun Age1720SituationQuestion(
    lesson: Lesson,
    answered: Boolean,
    isListening: Boolean,
    listen: ((String) -> Unit) -> Unit,
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            Text("상황을 보고 판단, 이유, 입장을 영어로 말하세요.", fontSize = 19.sp, fontWeight = FontWeight.Bold)
            ...
            if (recognized.isNotBlank()) {
                Text("인식: $recognized")
                Text("표현 일치도 $score%", fontWeight = FontWeight.Bold)
                Text("허용 예: ${lesson.accepted.joinToString(" / ")}", fontSize = 13.sp)
            }
        }
    }
}
...
@Composable
private fun Age1720AssessmentResult(
    modifier: Modifier,
    summary: Age1720AssessmentSummary,
    onSaveAndClose: () -> Unit,
    onRetry: () -> Unit
) {
    Column(
        ...
        Text("17~20세 과정 테스트 결과", fontSize = 30.sp, fontWeight = FontWeight.Bold)
        Text(
        ...
        Age1720ResultRow("듣기", summary.listeningCorrect, summary.label(summary.listeningCorrect))
        Age1720ResultRow("말하기", summary.speakingCorrect, summary.label(summary.speakingCorrect))
        Age1720ResultRow("어순", summary.orderCorrect, summary.label(summary.orderCorrect))
        Age1720ResultRow("상황·판단", summary.situationCorrect, summary.label(summary.situationCorrect))
        HorizontalDivider()
        Text("틀린 문항은 17~20세 자동 복습에 반영되었습니다.")
...
@Composable
private fun Age1720ResultRow(label: String, correct: Int, result: String) {
    Card(modifier = Modifier.fillMaxWidth()) {
        Row(
            Modifier.padding(16.dp),
...
private fun age1720MeaningChoices(lesson: Lesson): List<String> {
    val lessons = CourseCatalog.lessons(CourseLevel.AGE_17_20)
    val index = lessons.indexOfFirst { it.id == lesson.id }.coerceAtLeast(0)
    val choices = mutableListOf(
        lesson.meaning,
        lessons[(index + 31) % lessons.size].meaning,
        lessons[(index + 67) % lessons.size].meaning
...
private fun age1720Chunks(lesson: Lesson): List<Age1720AssessmentChunk> {
    ...
    val original = words.chunked(chunkSize)
        .mapIndexed { index, part -> Age1720AssessmentChunk(index, part.joinToString(" ")) }
    if (original.size <= 1) return original
    val shift = ((lesson.courseLessonNumber * 7) % (original.size - 1)) + 1
    return original.drop(shift) + original.take(shift)
}

private fun age1720SentenceMatch(spoken: String, lesson: Lesson): Int {
    val candidates = listOf(lesson.target) + lesson.accepted
    return candidates.maxOfOrNull { target ->
        val a = age1720Normalize(spoken)
        val b = age1720Normalize(target)
        if (a.isBlank() || b.isBlank()) {
            0
        } else {
            val distance = age1720Levenshtein(a, b)
...
private fun age1720Normalize(text: String): String =
    text.lowercase()
        .replace(Regex("[^a-z0-9' ]"), "")
        .replace(Regex("\\s+"), " ")
        .trim()

private fun age1720Levenshtein(a: String, b: String): Int {
