...
private data class Age810AssessmentChunk(
    val id: Int,
    val text: String
)
...
fun Age810AssessmentScreen(
    ...
    val summary = Age810AssessmentSummary(
    ...
    Scaffold(
        ...
        val question = Age810AssessmentCatalog.questions[currentIndex]
        ...
        Column(
            ...
            LinearProgressIndicator(
                progress = (currentIndex + 1).toFloat() / Age810AssessmentCatalog.questions.size,
                modifier = Modifier.fillMaxWidth()
            )
            Text(
                "문항 " + (currentIndex + 1) + " / " + Age810AssessmentCatalog.questions.size,
            ...
            when (question.kind) {
                AssessmentKind.LISTENING -> Age810ListeningQuestion(
                    lesson = lesson,
                    answered = answered,
                    speak = speak,
                    onAnswer = { correct ->
                ...
                AssessmentKind.SPEAKING -> Age810SpeakingQuestion(
                    lesson = lesson,
                    answered = answered,
                ...
                AssessmentKind.ORDER -> Age810OrderQuestion(
                    lesson = lesson,
                    answered = answered,
                    onAnswer = { correct ->
                        if (!answered) {
                ...
                AssessmentKind.SITUATION -> Age810SituationQuestion(
                    lesson = lesson,
                    answered = answered,
                    isListening = isListening,
                    listen = listen,
            ...
            if (answered) {
                ...
                Button(
                    onClick = {
                        if (currentIndex == Age810AssessmentCatalog.questions.lastIndex) {
                            finished = true
                        } else {
                            currentIndex += 1
                        }
...
@Composable
private fun Age810ListeningQuestion(
    lesson: Lesson,
    answered: Boolean,
    speak: (String) -> Unit,
    ...
    val choices = remember(lesson.id) { age810MeaningChoices(lesson) }
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            Text("문장이 길어졌으므로 핵심 내용과 이유 관계까지 들어보세요.")
...
@Composable
private fun Age810SpeakingQuestion(
    lesson: Lesson,
    answered: Boolean,
    isListening: Boolean,
    listen: ((String) -> Unit) -> Unit,
        ...
        Column(
            ...
            Button(
                onClick = {
                    listen { text ->
                        recognized = text
                        score = age810SentenceMatch(text, lesson)
                        onAnswer(score >= 75)
                    }
                },
...
@Composable
private fun Age810OrderQuestion(
    lesson: Lesson,
    answered: Boolean,
    onAnswer: (Boolean) -> Unit
) {
    val chunks = remember(lesson.id) { age810Chunks(lesson) }
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                Button(
                    onClick = {
                        ...
                        onAnswer(age810Normalize(candidate) == age810Normalize(lesson.target))
                    },
...
@Composable
private fun Age810SituationQuestion(
    lesson: Lesson,
    answered: Boolean,
    isListening: Boolean,
    listen: ((String) -> Unit) -> Unit,
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            Text("상황을 보고 이유·계획·생각까지 영어로 말하세요.", fontSize = 19.sp, fontWeight = FontWeight.Bold)
...
@Composable
private fun Age810AssessmentResult(
    modifier: Modifier,
    summary: Age810AssessmentSummary,
    onSaveAndClose: () -> Unit,
    onRetry: () -> Unit
) {
    Column(
        ...
        Text("8~10세 과정 테스트 결과", fontSize = 30.sp, fontWeight = FontWeight.Bold)
        Text(
        ...
        Age810ResultRow("듣기", summary.listeningCorrect, summary.label(summary.listeningCorrect))
        Age810ResultRow("말하기", summary.speakingCorrect, summary.label(summary.speakingCorrect))
        Age810ResultRow("어순", summary.orderCorrect, summary.label(summary.orderCorrect))
        Age810ResultRow("상황·추론", summary.situationCorrect, summary.label(summary.situationCorrect))

        HorizontalDivider()
        Text("틀린 문항은 8~10세 자동 복습에 이미 반영되었습니다.")
...
private fun Age810ResultRow(
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Row(
            ...
            Text(correct.toString() + " / 5")
            Text(result, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.Bold)
        }
    }
}

private fun age810MeaningChoices(lesson: Lesson): List<String> {
    val lessons = CourseCatalog.lessons(CourseLevel.AGE_8_10)
...
private fun age810Chunks(lesson: Lesson): List<Age810AssessmentChunk> {
    ...
    val original = words.chunked(chunkSize)
        .mapIndexed { index, part -> Age810AssessmentChunk(index, part.joinToString(" ")) }
...
private fun age810SentenceMatch(spoken: String, lesson: Lesson): Int {
    val candidates = listOf(lesson.target) + lesson.accepted
    return candidates.maxOfOrNull { target ->
        val a = age810Normalize(spoken)
        val b = age810Normalize(target)
        if (a.isBlank() || b.isBlank()) {
            0
        } else {
            val distance = age810Levenshtein(a, b)
...
private fun age810Normalize(text: String): String =
    text.lowercase()
        .replace(Regex("[^a-z0-9' ]"), "")
        .replace(Regex("\\s+"), " ")
        .trim()

private fun age810Levenshtein(a: String, b: String): Int {
